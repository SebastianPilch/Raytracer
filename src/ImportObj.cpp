...
void GetDataFromObj(map<int, point3> &vertices_coors, map<int, vector<int>> &vertices_to_faces, map<int, vec3> &faces_normals, map<int, Plane>& Planes_to_faces, string file_path)
{
    ifstream plik;
    plik.open(file_path);
    if (!plik.is_open()) {
    ...
    for (const auto& pair : vertices_coors) {
        cout << "Vertex: " << pair.first << ", Coordinates: ";
        cout << pair.second << " ";
        cout << endl;
    }
